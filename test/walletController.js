...
async function loadWeb3God() {
  window.web3God = new Web3("https://goerli.blockpi.network/v1/rpc/public");
...
async function afterConnectWallet() {
  ...
  console.log(`Current wallet: ${window.currentWallet}`);
  ...
  console.log("this is balance : " + result);
...
const networkHandler = async () => {
  try {
    await window.ethereum.request({
      ...
      params: [{ chainId: "0x5" }], //ethereum Mainnet
...
async function getBalance() {
  ...
  while (maxCounter >= 1) {
    try {
      ...
      ParticipateIds = [maxCounter].concat(ParticipateIds);
    } catch {}
...
async function _connectWithModal() {
  ...
  const providerOptions = {
    ...
    "custom-walletlink": {
      options: {
        appName: "RenQ",
        networkUrl: "https://goerli.blockpi.network/v1/rpc/public",
        rpc: {
          5: "https://goerli.blockpi.network/v1/rpc/public",
        },
        chainId: 5,
  ...
  window.provider.on("chainChanged", async (chainId) => {
    ...
    if (window.chainId !== 5) {
      window.alert(
        "Please change your network to Ethereum Mainnet to be able to interact with the dapp!"
      );
      await networkHandler();
      //   window.location.reload();
    }
    ...
    //   await networkHandler();
    // }
    //if chain didnt change send an alert and also dont connect the wallet
  });
}

async function connectWithModal() {
  //check chainId using window.ethereum
...
async function ClaimTokens() {
  ...
  if (ParticipateIds.length == 0) {
    alert("You have no participation");
    return;
  }
  const prev = claimButton.innerHTML;
